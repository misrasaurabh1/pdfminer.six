...
/// Returns list of (objid, genno, offset, is_in_use) tuples for every entry.
///
/// PDF files may use `\r`, `\n`, or `\r\n` as line endings. Rust's
/// `str::lines()` does NOT split on lone `\r`, so we normalise first.
#[pyfunction]
pub fn parse_xref_table(data: &[u8]) -> PyResult<Vec<(u64, u64, u64, bool)>> {
    // Normalise line endings: replace lone \r (not followed by \n) with \n.
    // We work on bytes to avoid an extra allocation for already-UTF-8 data.
    let normalised: Vec<u8> = normalize_line_endings(data);
    let text = std::str::from_utf8(&normalised)
...
/// Replace lone `\r` (CR not followed by `\n`) with `\n`.
/// `\r\n` sequences are left as-is (Rust `lines()` handles those).
...
pub fn scan_pdf_objects(data: &[u8]) -> PyResult<Vec<(u64, u64, u64)>> {
    ...
    while pos < data.len() {
        // Find end of this line (LF-terminated; CR+LF handled by trim_start)
...
/// Try to parse `<digits> <digits> obj[<non-word>|EOF]` from a line.
fn try_match_obj_header(line: &[u8]) -> Option<(u64, u64)> {
    // Trim leading whitespace (space, tab, CR)
    ...
    // Must start with "obj" at a word boundary
...
pub fn parse_xref_stream(
    ...
    for chunk in index.chunks(2) {
        ...
        for i in 0..count {
            ...
            // w1 field: entry type (default = 1 if w1 == 0 per PDF spec)
            let entry_type: u8 = if w1 == 0 {
                1
            } else {
                read_be_field(&data[data_pos..data_pos + w1]) as u8
            };
...
/// Read a big-endian unsigned integer from a byte slice (0–8 bytes).
#[inline]
